...
app.post('/api/versions/:versionId/switch', (req, res) => {
  try {
    ...
    if (setCurrentVersion(versionId)) {
      ...
      res.json({ success: true, message: 'Versión activada. Por favor, recarga la página.' });
    } else {
      res.status(400).json({ error: 'Error al cambiar de versión' });
    }
  } catch (error) {
    res.status(500).json({ error: 'Error al cambiar de versión' });
  }
});
