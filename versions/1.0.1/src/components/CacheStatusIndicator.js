  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <button
        ...
        aria-label="Mostrar estado del caché"
      >
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-72 bg-white p-6 rounded-xl shadow-2xl border border-gray-200 transform transition-all duration-300 scale-100 opacity-100 origin-bottom-right">
          <h4 className="text-xl font-bold text-gray-900 mb-4">Estado del Caché</h4>
          <div className="mb-4">
            <p className="text-lg text-gray-700">Elementos en caché: <span className="font-semibold">{cacheInfo.size} / {cacheInfo.maxSize}</span></p>
          ...
          <div>
            <p className="text-md font-semibold text-gray-800 mb-2">Chunks Almacenados:</p>
            <ul className="list-disc list-inside text-gray-600 text-sm max-h-24 overflow-y-auto custom-scrollbar">
              {cacheInfo.keys.length > 0 ? (
                cacheInfo.keys.map((key) => (
                  <li key={key} className="truncate">{key}</li>
                ))
              ) : (
                <li>Ningún chunk en caché.</li>
              )}
            </ul>
          </div>
        </div>
